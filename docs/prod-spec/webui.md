...
> **版本**：rev 12 · **最近修订**：2026-04-28 · **状态**：active
>
...
| `/ui/tasks/{id}` | React source 详情：URL 数量、depth 分布、frontier 状态；用单个 URL 列表承载全部 URL，通过 Tabs 切换“全部 / 已采集 / 未采集”；列表内容列只显示操作按钮或状态，不显示正文/摘要；已采集行不外跳源站，进入站内采集详情 |
| `/ui/tasks/{id}/items/{raw_id}` | React 采集详情：展示本系统入库的标题、正文、source metadata、raw blob URI、源 URL 文本；入库信息 label 固定列宽对齐；详情页发现的 depth+1 子链接直接展开在入库信息 `Attachments` 下方，附件行必须优先显示可点击的附件名称 |
...
- `/tasks/{id}` 用一个 URL 列表展示 source 结果：Tabs 切换“全部 / 已采集 / 未采集”；“已采集”定义为已写入 `crawl_raw` 且有 `raw_id`，内容列只显示“查看采集详情”按钮，不显示标题/正文摘要；“未采集”定义为尚无 `crawl_raw` 的 URL（包含 pending URL 与已 fetch 但未生成内容的列表页）。
...
| `GET /api/tasks/{id}/urls?kind=all|collected|uncollected|fetched|jump&depth=&limit=50&offset=0` | 单 source URL 明细：total / limit / offset / depth_summary / items；`collected` 返回已写入 `crawl_raw` 的采集结果，`uncollected` 返回尚无 `crawl_raw` 的 URL；`fetched` / `jump` 保留兼容旧前端 | viewer |
| `GET /api/tasks/{id}/items/{raw_id}` | 单条 `crawl_raw` 采集详情：title / body_text / source_metadata / attachments / raw_blob_uri / source URL / child_links；`child_links` 包含附件、解读/图解、其它详情页内发现链接 | viewer |
...
- 点进 `/tasks/{id}` 后能看到该 source 的 URL 数量、depth 分布；URL 列表必须是单表，通过“全部 / 已采集 / 未采集”Tabs 切换并服务端分页；列表不展示内容全文/摘要；已采集行不直接跳源站，只显示按钮进入站内详情页展示本系统抓取内容
- 点进 `/tasks/{id}/items/{raw_id}` 后能看到正文、入库信息、metadata，以及该详情页发现的 depth+1 子链接；入库信息 label 固定列宽对齐；附件和“相关政策与解读”等不能只停留在正文里，必须在 `Attachments` 下直接展开；附件行优先显示可点击附件名称，其次显示可复制 URL
...
| rev 8 | 2026-04-28 | 已抓取链接改为进入站内采集详情页，不直接跳转源站；新增 React `/ui/tasks/{id}/items/{raw_id}` 与 `GET /api/tasks/{id}/items/{raw_id}` 展示 `crawl_raw` 内容 | 用户决策 2026-04-28 |
| rev 9 | 2026-04-28 | 任务详情页从两张 URL 表改为单表 + Tabs（全部 / 已采集 / 未采集）；新增 `kind=collected|uncollected`，以 `crawl_raw` 是否存在定义采集完成状态 | 用户决策 2026-04-28 |
| rev 10 | 2026-04-28 | URL 列表内容列改为仅显示操作按钮或状态，不在列表中展示采集正文/摘要，正文仅在站内详情页展示 | 用户决策 2026-04-28 |
| rev 11 | 2026-04-28 | 采集详情页新增 depth+1 子链接展示；API `GET /api/tasks/{id}/items/{raw_id}` 返回 `child_links`，包含附件、相关政策与解读等详情页内发现链接；附件行优先显示附件名称 | 用户决策 2026-04-28 |
| rev 12 | 2026-04-28 | 采集详情页布局调整：入库信息 label 固定列宽对齐；depth+1 子链接不再单独成表，直接展开在 `Attachments` 下，附件名称优先展示并可点击打开 | 用户决策 2026-04-28 |
