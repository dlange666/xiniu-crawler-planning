...
| 巡检 | 触发 | 动作 |
|---|---|---|
| **黄金 fixture 回放** | cron 每日 03:00 | 用本 source 的 `tests/domains/<context>/<source>/fixtures/<source>_golden_*.html` 跑解析；输出与期望 JSON 不等 → webhook + 自动开 fix-task（`task_type=update`） |
...
- 在 `tests/domains/<context>/<source>/fixtures/<source>_golden_*` 维护快照
...
| 修订 | 日期 | 摘要 | 关联 |
|---|---|---|---|
| rev 4 | 2026-04-29 | 版本巡检和业务域接口中的 golden fixture 路径迁移到 `tests/domains/<context>/<source>/fixtures/` | `codegen-output-contract.md` rev 16 |
