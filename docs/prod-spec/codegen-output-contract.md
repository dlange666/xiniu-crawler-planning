...
> **版本**：rev 13 · **最近修订**：2026-04-29 · **状态**：active
...
5. **能力缺口 fallback**：当 infra helper 不覆盖但页面存在明确、稳定、可静态解析的分页/API/字段信号时，coding agent 不得直接判 red 或 `render_required`；必须先在当前 source 的 domain adapter 内实现有界 fallback，并用 golden/test/eval 记录。codegen 任务仍不得修改 `infra/`；可复用能力由单独 infra 任务提升。
6. **未来自动化**：本规约目前由调用方人工执行；`infra/codegen/` worker 落地后必须接管 1–5 步，使其不可绕过（spec `codegen-bootstrap`）。
