...
6. **受控探查工具**（rev 6 引入）：coding agent 不得自行把探查样本写到
...
| 修订 | 日期 | 摘要 | 关联 |
|---|---|---|---|
| rev 6 | 2026-04-29 | 新增受控 source probe 调用规约：codegen 必须先用 `scripts/probe_source.py` 在 repo `runtime/probe/` 下留存 artifact，再选择 static / JSON API / headless_required；禁止把探查样本写 `/tmp` 或在 hook 内联网 | `scripts/probe_source.py`、`infra/source_probe/`、`docs/codegen-pipeline.md` |
