...
> **版本**：rev 17 · **最近修订**：2026-04-29 · **状态**：active
...
源目录：`domains/<context>/<source>/`。同一源的 adapter 与 seed 放在该运行时
目录；golden HTML/JSON 是版本化测试夹具，放在
`tests/domains/<context>/<source>/fixtures/`，避免把测试数据混入 domain 生产
代码目录。
...
| 产物 | 路径 | 数量 / 要求 |
|---|---|---|
| Golden HTML | `tests/domains/<context>/<source>/fixtures/<source>_golden_*.html` | 覆盖型固定快照：≥1 list、≥3 detail；有分页信号时 ≥1 pagination/list_2 |
| Golden JSON | `tests/domains/<context>/<source>/fixtures/<source>_golden_*.golden.json` | 与 HTML 一一同名配对，禁止用聚合 JSON 替代配对样本 |
| 单元测试 | `tests/domains/<context>/<source>/test_adapter.py` | 覆盖 registry、list、detail、metadata、pagination（如适用） |
...
7. **未来自动化**：本规约目前仍主要由 wrapper 调用；`infra/codegen/` 基础
   worker 已落地 `run_once` 主循环，后续任务必须逐步接管 1–6 步，使其不可绕过。
...
9. **提交边界**：wrapper gates 结束后自动提交并推送 codegen 分支。`green`
   提交 plan/task/eval/adapter/seed/golden fixture/test 全量交付物；`red` 只提交 eval
   诊断报告，不提交半成品 adapter/test/golden。PR 创建和 merge 仍保留在人审边界。
...
| 单元测试 | `tests/domains/<context>/<source>/test_adapter.py` 存在且 `pytest` 全绿 |
...
当前基础实现包括 `infra/agent/backends.py`（`CodingAgentBackend`、`OpenCodeBackend`、
`MockAgentBackend`）、`infra/harness/`（gate/result/command runner/合规扫描）、
`infra/codegen/`（`TaskSource`、`CodegenWorker.run_once`）。prompt 模板文件仍由
后续任务补齐。
...
| 修订 | 日期 | 摘要 | 关联 |
|---|---|---|---|
| rev 17 | 2026-04-29 | 将 codegen 平台基础抽象下沉到 `infra/agent`、`infra/harness`、`infra/codegen`：agent backend、harness gate/result、TaskSource 与 worker run-once；现有 wrapper 后续逐步迁移 | `plan-20260429-render-codegen-platform` / T-20260429-1202 |
| rev 16 | 2026-04-29 | 将 golden HTML/JSON 从 domain 运行时代码目录迁移为 `tests/domains/<context>/<source>/fixtures/` 测试夹具；adapter 测试同步迁移为 `tests/domains/<context>/<source>/test_adapter.py`；codegen wrapper 通过 `infra/adapter_contract/golden.py` 复用配对/覆盖校验 | `plan-20260429-golden-fixture-layout` / T-20260429-1101 |
| rev 15 | 2026-04-29 | 明确 codegen wrapper 的自动提交边界：green 自动提交并推送完整 codegen 产物分支；red 只提交并推送 eval 诊断报告，避免半成品 adapter 入史；PR/merge 仍保留人审 | `scripts/run_codegen_for_adapter.py`、`docs/codegen-pipeline.md` |
