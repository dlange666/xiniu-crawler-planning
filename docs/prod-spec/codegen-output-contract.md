...
> **版本**：rev 8 · **最近修订**：2026-04-29 · **状态**：active
...
`<host>` 文件名使用 wrapper 计算出的 canonical host slug，必须承载源站主体
职责，不得直接使用 `www`、`wap`、`m`、`mobile` 等通用渠道前缀；例如
`wap.miit.gov.cn` 写入 `adapters/miit.py`，`search.sh.gov.cn` 写入
`adapters/sh_search.py`。
...
| 单元测试 | `tests/<context>/test_adapter_<host>.py` | 跑 5 个 golden 用例全绿 |
...
| rev 5 | 2026-04-28 | 将 `docs/codegen-brief.md` 升级并更名为 `docs/codegen-pipeline.md`；明确 codegen 模拟 workflow-execution 的顺序：git-worktree → plan → task → code/gates → eval → PR handoff → merge 边界 → notify-message 草稿；消除“禁止写 docs”与 Plan/Task/Eval 三件套之间的冲突；单元测试路径收敛为现行 `tests/<context>/test_adapter_<host>.py` | `docs/codegen-pipeline.md`、`scripts/run_codegen_for_adapter.py` |
