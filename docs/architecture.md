...
├── webui/                         任务后台 + 采集监控 + 结果浏览（FastAPI + Jinja2）
└── docs/
...
|---|---|---|---|---|
| 通用爬虫引擎 | `prod-spec/infra-crawl-engine.md` | `plan-20260427-mvp-policy-crawler` T-106..109 | `infra/crawl/` | MVP 已实现核心（rev 1） |
...
| Webui · 任务后台 + 监控 + 浏览 | `prod-spec/webui.md` | `plan-20260428-webui-bootstrap`（待立项） | `webui/`（顶层，待建） | MVP 实施（OAuth 暂缓 TD-018） |
...
- **Webui（任务后台 + 采集监控 + 结果浏览）**：spec `docs/prod-spec/webui.md` rev 2。顶层 `webui/` 项目，FastAPI + Jinja2 + Chart.js，三类 UI 路由分组合一；鉴权用 `AuthBackend` 抽象（MVP 用 `DevBackend` 免登；OAuth/OIDC 暂缓 TD-018），不在本仓库存储凭据；写操作落 `crawl_task.created_by` + `webui_audit` 表。
