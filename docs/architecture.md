...
| Webui · 任务后台 + 监控 + 浏览 | `prod-spec/webui.md` | `plan-20260428-webui-bootstrap` T-301/302 | `webui/` + `webui/frontend/` | React + Ant Design Pro 重写中（OAuth 暂缓 TD-018） |
...
- **Webui（任务后台 + 采集监控 + 结果浏览）**：spec `docs/prod-spec/webui.md` rev 7。FastAPI 提供 `/api/*`、鉴权和审计，并托管 `/ui` React SPA；前端位于 `webui/frontend/`，使用 React + TypeScript + Vite + Ant Design ProComponents。任务详情页把已抓取链接（fetch/raw 结果）与跳转/发现链接（pending frontier）分开展示并各自分页。鉴权用 `AuthBackend` 抽象（MVP 用 `DevBackend` 免登；OAuth/OIDC 暂缓 TD-018），不在本仓库存储凭据；写操作落 `crawl_task.created_by` + `webui_audit` 表。
