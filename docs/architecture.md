...
- **可观测性**：已 spec 化 `docs/prod-spec/observability.md`（零自建后台版：metric_snapshot + cron 告警 + LiteLLM 成本）；**MVP 暂缓实施**（TD-013）。
...
- **跳过人审的安全网**：分级合并 tier 1/2/3 + 加压 harness（golden ≥10、E2E ≥20、schema ≥98%、关键字段 ≥99%）+ 4 档渐进 canary + 自动回滚 + IM 审计；tier-3 路径永远人审。详见 `docs/prod-spec/auto-merge-policy.md`。
