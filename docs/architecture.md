...
| 政策业务规格 | `prod-spec/domain-gov-policy.md` | `plan-20260427-mvp-policy-crawler` (M1–M3) | `domains/gov_policy/<source>/`（adapter + seed）+ `tests/domains/gov_policy/<source>/fixtures/`（golden） | MVP 实施 |
...
| Headless 渲染池 | `prod-spec/infra-render-pool.md` | `plan-20260429-render-codegen-platform` T-20260429-1201；M5 后续见 deferred plan | `infra/render/`、`infra/crawl/runner.py` | 基础 decision/pool 已实现；真实 Playwright/queue 暂缓 |
...
| Codegen 平台 · adapter 输出契约 | `prod-spec/codegen-output-contract.md` | `plan-20260429-render-codegen-platform` T-20260429-1202；`plan-20260428-codegen-bootstrap` 后续 | `infra/agent/`、`infra/harness/`、`infra/adapter_registry/` | 基础 agent/harness 已实现；wrapper 迁移中 |
| Codegen 平台 · 自动合并安全网 | `prod-spec/codegen-auto-merge.md` | `plan-20260429-render-codegen-platform` T-20260429-1202；`plan-20260428-codegen-bootstrap` 后续 | `infra/sandbox/`、`infra/scheduler/`、`infra/codegen/` | 基础 sandbox/codegen worker 已实现；canary/rollback 暂未实现 |
...
- **adapter 路径**：`domains/<context>/<source>/<source>_adapter.py`。同一源的 seed 放在 source 运行时目录；golden fixture 放在镜像测试目录 `tests/domains/<context>/<source>/fixtures/`；不再使用 `adapters/`、`seeds/`、`golden/` 横向目录。
