...
- 不 import 其它业务域；不修改已有 adapter；不修改 `infra/` 内的 capability 模块（`infra/adapter_registry/`、`infra/crawl/`、`infra/storage/` 等），也不改 `AGENTS.md` / `CLAUDE.md` / `pyproject.toml`。
- `infra/codegen/` 是 wrapper 自身代码，agent 任务范围之外。
