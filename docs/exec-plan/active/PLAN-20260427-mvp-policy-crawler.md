...
| T-20260427-107 | [domains/gov_policy/{crawl,adapters}] 国务院文件库适配器 | 在 `adapters/statecouncil.py` 写：列表页分页、详情页 URL、DOM 选择器；通用 `crawl` 编排器调用之；下载详情页 HTML + 同页元数据表；附件 PDF 仅落盘 OSS（dev 走本地）；不做 PDF→文本 | 跑 100 条政策，全部原始字节通过 `infra/storage` 落盘；元数据表 + 正文 + 附件清单可被回放查询 | `pending` |
| T-20260427-108 | [domains/gov_policy/{parse,golden}] 解析框架 + 国务院黄金用例 | 通用 `parse/` 调度 `adapters` hook；输出 `model.PolicyParsed`；`golden/statecouncil/` 放 5 个固定快照与期望 JSON | 单元测试：5 个黄金用例全绿；解析结果与期望 JSON 一致 | `pending` |
...
| T-20260427-110 | [domains/gov_policy/adapters] 8 个国务院部委适配器 | 复用 §107/§108 模式，在 `adapters/{ndrc,miit,mof,nfra,csrc,sasac,pbc,other}.py` 各写一份；同时补 `golden/<host>/` | 每部委跑 50 条，原始页落盘 100% + 黄金用例全绿 + 解析合格率 ≥ 95% | `pending` |
...
| T-20260427-112 | [domains/gov_policy/{extract,sink}] 抽取流水线 | 把 36 字段 prompt 文本放在 `extract/prompts/v1.md`、schema 放在 `extract/schemas/policy_doc_v1.json`（业务事实）；从 store 拉解析后政策文本 → 调 `infra/ai` → 校验 → 通过 `sink` 写 `policy_doc` 表 | 跑 100 条政策，schema 合格率 ≥ 90%，关键 6 字段联合准确率 ≥ 95%（人工抽样 30 条） | `pending` |
| T-20260427-113 | [domains/gov_policy/harness_rules] 业务侧 harness 规则占位 | 暂留空 stub `harness_rules.py`，定义 `compliance_blocklist`、`field_hit_thresholds` 接口；M3.5 codegen-bootstrap 时由 `infra/harness` 调用 | 占位文件存在；import 不报错；单元测试不覆盖（M3.5 一并验收） | `pending` |
