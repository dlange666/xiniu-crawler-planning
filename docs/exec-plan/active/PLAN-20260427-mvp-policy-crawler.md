...
| T-20260427-107 | [domains/gov_policy/{crawl,adapters}] 国务院文件库适配器 | `codegen-output-contract.md` §2, `domain-gov-policy.md` §8 | 在 `adapters/statecouncil.py` 写：列表页分页、详情页 URL、DOM 选择器；通用 `crawl` 编排器调用之；下载详情页 HTML + 同页元数据表；附件 PDF 仅落盘 OSS（dev 走本地）；不做 PDF→文本 | 跑 100 条政策，全部原始字节通过 `infra/storage` 落盘；元数据表 + 正文 + 附件清单可被回放查询 | `pending` |
| T-20260427-108 | [domains/gov_policy/{parse,golden}] 解析框架 + 国务院黄金用例 | `codegen-output-contract.md` §2.2, §3 | 通用 `parse/` 调度 `adapters` hook；输出 `model.PolicyParsed`；`golden/statecouncil/` 放 5 个固定快照与期望 JSON | 单元测试：5 个黄金用例全绿；解析结果与期望 JSON 一致 | `pending` |
...
| T-20260427-110 | [domains/gov_policy/adapters] 8 个国务院部委适配器 | `codegen-output-contract.md` §2 | 复用 §107/§108 模式，在 `adapters/{ndrc,miit,mof,nfra,csrc,sasac,pbc,other}.py` 各写一份；同时补 `golden/<host>/` | 每部委跑 50 条，原始页落盘 100% + 黄金用例全绿 + 解析合格率 ≥ 95% | `pending` |
