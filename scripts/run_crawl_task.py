...
from domains.gov_policy.crawl import CrawlRunner  # noqa: E402
from domains.gov_policy.model import TaskSpec  # noqa: E402
from domains.gov_policy.seeds import load_seed  # noqa: E402
...
def main() -> int:
    ...
    task = TaskSpec(
        ...
        data_kind="policy",
        max_pages_per_run=seed.max_pages_per_run,
        politeness_rps=seed.politeness_rps,
        crawl_mode=seed.crawl_mode,
    )
    print("\n=== Crawl Task ===")
    print(f"  task_id    = {task.task_id}")
    print(f"  host       = {seed.host}")
    print(f"  entry      = {seed.entry_urls}")
    print(f"  max_pages  = {seed.max_pages_per_run}")
    print(f"  RPS        = {seed.politeness_rps}")
    print()

    runner = CrawlRunner(task=task, seed=seed)
    try:
        report = runner.run()
    finally:
        runner.close()
