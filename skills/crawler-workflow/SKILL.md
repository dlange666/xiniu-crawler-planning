...
新建 spec 必须用 `docs/prod-spec/template.md` 起手。
...
- `crawler-workflow-experiment`：实验环路 `Hypothesis -> Setup -> Execution -> Artifact -> Communication -> Closure`
...
- 任务是探索性策略/参数/prompt 调优，且能用 additive candidate 在临时 DB 上跑出 retained-vs-candidate 工件 → `crawler-workflow-experiment`
